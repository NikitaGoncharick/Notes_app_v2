...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        noteAdapter.setOnNoteClickListener(new NoteAdapter.OnNoteClickListener() {
            //Определяем, что должно произойти, когда пользователь кликнул на элемент RecyclerView

            //происходит клик, посредством вызова noteAdapter.setOnNoteClickListener мы передем в адаптер ..иструкцию.. того, как будет реализован интерфейс OnNoteClickListener(код ниже)
            //в последствии, ..инстукция.. сделает обращение к onNoteClick и реализует код

            // Как интерфейс OnNoteClickListener понимает что мы передает реализацию логики для его класса onNoteClick:
            // Создание Анонимного Класса: Вы создаете анонимный класс, который реализует интерфейс OnNoteClickListener. Этот анонимный класс содержит вашу конкретную реализацию метода onNoteClick.
            //Передача в Адаптер: Вы передаете этот анонимный класс в адаптер, который сохраняет его как listener.
            //Вызов метода: Когда происходит клик, адаптер использует этот сохраненный listener для вызова метода onNoteClick.
            @Override
